...
class _UserFormViewState extends State<UserFormView> {
  ...
  void initState() {
    ...
    // carrega módulos se vier da API
    if (widget.user?['modules'] != null) {
      try {
        final List<dynamic> mods = widget.user!['modules'] is String
            ? jsonDecode(widget.user!['modules'])
            : List.from(widget.user!['modules']);
        for (var m in mods) {
          if (_modules.containsKey(m)) _modules[m] = true;
        }
      } catch (_) {}
    }
  }
  ...
  Future<void> _salvar() async {
    ...
    try {
      ...
      // monta lista de módulos ativos
      final activeModules = _modules.entries
          .where((e) => e.value)
          .map((e) => e.key)
          .toList();
          ...
          children: [
            ...
            ..._modules.keys.map((key) {
              return CheckboxListTile(
                ...
                activeColor: const Color(0xFF1B286C),
                value: _modules[key],
                onChanged: (v) => setState(() => _modules[key] = v ?? false),
              );
            }).toList(),
            const SizedBox(height: 20),
            SizedBox(
